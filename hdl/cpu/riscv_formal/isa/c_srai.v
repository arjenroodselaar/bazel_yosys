...
module rvfi_instruction_c_srai (
  input                                 rvfi_valid,
...
`endif
  ...
  wire [5:0] instruction_shamt = {rvfi_instruction[12], rvfi_instruction[6:2]};
  wire [2:0] instruction_funct3 = rvfi_instruction[15:13];
  wire [1:0] instruction_funct2 = rvfi_instruction[11:10];
...
`endif
  ...
  // C_SRAI instruction
  wire [`RISCV_FORMAL_XLEN-1:0] result = $signed(rvfi_rs1_rdata) >>> instruction_shamt;
  assign spec_valid = rvfi_valid && !instruction_padding && instruction_funct3 == 3'b 100 && instruction_funct2 == 2'b 01 && instruction_opcode == 2'b 01 && (!instruction_shamt[5] || `RISCV_FORMAL_XLEN == 64);
